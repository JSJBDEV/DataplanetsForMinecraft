...
public class NoiseModel {
    ...
    public double sample(GeneratorType type, Vec3 p) {
        return switch (type) {
            case WORLEY -> worley(p, 4.0);
            case STRIPES -> stripes(p, 6.0, 0.06);   // gentle warp amplitude
            case HYBRID -> hybrid(p, 6.0, 0.06, 4);
            default -> fractal(p, 6);
        };
    }
    ...
    private double perlin(double x, double y, double z) {
        ...
        return Maths.lerp(w,
                Maths.lerp(v,
                        Maths.lerp(u, grad(aaa, x,   y,   z  ), grad(baa, x-1, y,   z  )),
                        Maths.lerp(u, grad(aba, x,   y-1, z  ), grad(bba, x-1, y-1, z  ))),
                Maths.lerp(v,
                        Maths.lerp(u, grad(aab, x,   y,   z-1), grad(bab, x-1, y,   z-1)),
                        Maths.lerp(u, grad(abb, x,   y-1, z-1), grad(bbb, x-1, y-1, z-1))));
    }
