...
public class PaletteMapper {
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                ...
                if (dither) {
                    ...
                    r = Maths.clamp(r + offset);
                    g = Maths.clamp(g + offset);
                    b = Maths.clamp(b + offset);
                }
    ...
    public PaletteColor lerp(PaletteColor a, PaletteColor b, float t) {
        int r  = Maths.clamp(Math.round(a.r + t * (b.r - a.r)));
        int g  = Maths.clamp(Math.round(a.g + t * (b.g - a.g)));
        int bv = Maths.clamp(Math.round(a.b + t * (b.b - a.b)));
