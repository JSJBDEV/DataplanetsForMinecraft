...
public class SeamFixer {
    ...
    private static int[] getEdgePixel(BufferedImage img, int edge, int i, int depth, int res) {
        int x = 0, y = 0;
        switch (edge) {
        ...
        int rgb = img.getRGB(Maths.clamp(x, res), Maths.clamp(y, res));
        return new int[]{(rgb>>16)&0xFF, (rgb>>8)&0xFF, rgb&0xFF};
    }

    private static void setEdgePixel(BufferedImage img, int edge, int i, int depth, int res, int[] color) {
        int x = 0, y = 0;
        switch (edge) {
        ...
        img.setRGB(Maths.clamp(x, res), Maths.clamp(y, res),
                (color[0] << 16) | (color[1] << 8) | color[2]);
    }

    private static int[] blend(int[] a, int[] b, float t) {
        return new int[]{
            Maths.clamp255(Math.round(a[0] + t * (b[0] - a[0]))),
            Maths.clamp255(Math.round(a[1] + t * (b[1] - a[1]))),
            Maths.clamp255(Math.round(a[2] + t * (b[2] - a[2])))
        };
    }
