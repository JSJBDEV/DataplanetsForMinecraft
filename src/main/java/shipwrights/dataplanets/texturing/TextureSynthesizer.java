...
public class TextureSynthesizer {
    ...
    private void synthesizeFace(BufferedImage face, int faceIdx, int res) {
        ...
        for (int y = 0; y < res; y++) {
            ...
            for (int x = 0; x < res; x++) {
                ...
                if (val < 0.0) val = 0.0;
                if (val > 1.0) val = 1.0;
                int noiseGrey = Maths.clamp((int) (val * 255));

                int pr, pg, pb;
                if (totalW[y][x] > 0) {
                    pr = Maths.clamp((int)(weightR[y][x] / totalW[y][x]));
                    pg = Maths.clamp((int)(weightG[y][x] / totalW[y][x]));
                    pb = Maths.clamp((int)(weightB[y][x] / totalW[y][x]));
                } else {
                    pr = pg = pb = noiseGrey;
                }

                int r = Maths.lerp(noiseGrey, pr, PATCH_BLEND);
                int g = Maths.lerp(noiseGrey, pg, PATCH_BLEND);
                int b = Maths.lerp(noiseGrey, pb, PATCH_BLEND);

                int detail = detailOffset(dir);
                r = Maths.clamp(r + detail);
                g = Maths.clamp(g + detail);
                b = Maths.clamp(b + detail);
    ...
    private record AxisBlocks(int[] start, int[] size) {
    }
}
